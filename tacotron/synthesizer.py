...
class Synthesizer:
	...
	def synthesize(self, texts, speaker_labels, language_labels, basenames, out_dir, log_dir, mel_filenames):
		...
		if self.gta or not hparams.predict_linear:
			...
			speaker_table, linears, mels, alignments, stop_tokens = self.session.run([self.speaker_table, self.linear_outputs, self.mel_outputs, self.alignments, self.stop_token_prediction], feed_dict=feed_dict)
			#Linearize outputs (1D arrays)
